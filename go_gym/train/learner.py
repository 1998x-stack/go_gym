...
from ..train.evaluator import arena, EvalConfig   # ← 引入评测
...
def train_step(batch, model, optim, scaler, device, cfg: OptimConfig, step: int) -> Dict[str, float]:
    ...
    with autocast(enabled=bool(cfg.amp)):
        logits, v = model(planes)               # [B,A], [B,1]
        logp = torch.log_softmax(logits, dim=-1)
        policy_loss = (- (pi * logp).sum(dim=-1)).mean()     # soft CE
...
def learner_loop(
    ...
    # 类型强转，避免 YAML 字符串数值
    lr = float(optim_cfg.lr); wd = float(optim_cfg.weight_decay)
    betas = (float(optim_cfg.betas[0]), float(optim_cfg.betas[1])); amp_enabled = bool(optim_cfg.amp)

    optim = AdamW(model.parameters(), lr=lr, weight_decay=wd, betas=betas)
    ...
    while step < int(max_steps):
        ...
        if eval_cfg is not None and best_model is not None and (step + 1) % gate_every == 0:
            ...
            if winrate >= float(eval_cfg.games and eval_cfg.gate_winrate or 0.55):  # 容错
                # 通过门控：提升 best & 广播给自博弈
                ...
                logger.success(f"[GATE PASS] winrate={winrate:.3f} >= {eval_cfg.gate_winrate:.3f} → Promote BEST & broadcast to selfplay.")
            else:
                logger.warning(f"[GATE FAIL] winrate={winrate:.3f} < {getattr(eval_cfg, 'gate_winrate', 0.55):.3f}")
